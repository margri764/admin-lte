      ...
      ( {success, user})=>{
        if(success){
          ...
          if(user.linkCongregatio === 1){
            const {iduser, validateEmail, name, lastName, role, codeauth, state, active, simpleCode, linkCongregatio, Nome_da_sede, Pais_da_sede, Cidade_da_sede,  ...userModified } = user;
